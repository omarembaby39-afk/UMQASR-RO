...
def init_postgres():
    ...
    # Clean any wrong rows (like 'name')
    cur.execute("""
        DELETE FROM chemicals
        WHERE name NOT IN ('Chlorine','HCL','BC');
    """)
...
def render_dp_gauge(dp_value):
    ...
    # Colors with 1 / 2 / 3 bar thresholds
    if dp < 1:
...
def page_dashboard():
    ...
    last_ph_value = float(last["ph"])
    last_ph = f"{last_ph_value:.2f}"

    # pH KPI colour:
    # 7–8 green, 6.5–7 or 8–8.5 amber, else red
    ph_color = "#E74C3C"
    if 7.0 <= last_ph_value <= 8.0:
        ph_color = "#27AE60"
    elif (6.5 <= last_ph_value < 7.0) or (8.0 < last_ph_value <= 8.5):
        ph_color = "#F1C40F"

    df30 = df[df['d'] >= datetime.now() - pd.Timedelta(days=30)]
    if len(df30) > 0:
        compliance, good_days, bad_days = compute_compliance(df30)
        comp_val = f"{compliance:.1f}%"
    else:
        comp_val, good_days, bad_days = "0%", 0, 0

    # this month production
    month_prod = 0.0
    df["month"] = df["d"].dt.strftime("%Y-%m")
    current_month = datetime.now().strftime("%Y-%m")
    dfm = df[df["month"] == current_month]
    if len(dfm) > 0:
        month_prod = dfm["production"].sum()

    col1, col2, col3, col4 = st.columns(4)
    with col1: kpi_card("Unit Capacity", "10 m³/hr")
    with col2: kpi_card("Last TDS", last_tds)
    with col3: kpi_card("Last pH", last_ph, color=ph_color)
    with col4: kpi_card("Total Records", len(df))

    col5, col6, col7, col8 = st.columns(4)
    with col5: kpi_card("30-day Compliance", comp_val)
    with col6: kpi_card("In-Spec Days (30d)", good_days)
    with col7: kpi_card("Out-of-Spec (30d)", bad_days)
    with col8: kpi_card("This Month Production", f"{month_prod:.1f} m³")
    ...
    if len(df30) > 0:
        ...
        ax.plot(df30["d"], df30["tds"], label="TDS (ppm)", linewidth=2)
        ax.plot(df30["d"], df30["ph"], label="pH", linewidth=2)
        ax.plot(df30["d"], df30["conductivity"], label="Conductivity", linewidth=2)
    ...
    if len(cart) == 0:
        ...
        if latest_dp < 1:
            ...
            st.warning("1–2 bar – monitor filter.")
        elif latest_dp < 3:
            st.warning("2–3 bar – high DP, prepare to change.")
        else:
            st.error("DP ≥ 3 bar – change cartridge now.")
    ...
    if len(chem) == 0:
        ...
        st.dataframe(
            chem_df[["display_name", "qty", "unit_cost", "stock_value"]]
            .rename(columns={
        ...
        # Simple donut stock distribution
        ...
        for _, row in chem_df.iterrows():
            nm = row["display_name"]
            qty = float(row["qty"])
            min_level = CHEMICAL_SETTINGS.get(row["name"], {}).get("min_level", 0)

            if qty <= 0:
                st.error(f"🔴 {nm}: stock EMPTY (0 kg) – urgent reorder.")
            elif qty < min_level:
                st.warning(f"🟠 {nm}: low stock ({qty:.1f} kg, below {min_level} kg).")
            else:
...
def page_chemicals():
    ...
    with tab1:
        ...
        if len(chem_df) == 0:
            ...
            with c1:
                # radio – 3 inline options, no dropdown
                chem_name = st.radio(
                    "Chemical",
                    ALLOWED_CHEMICALS,
                    horizontal=True,
    ...
    with tab2:
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            d = st.date_input("Date", value=date.today(), key="chem_date2")
        with c2:
            # radio again – HCL / BC / Chlorine inline
            name = st.radio(
                "Chemical",
                ALLOWED_CHEMICALS,
                horizontal=True,
                key="chem_name_select2",
            )
        with c3:
            movement_type = st.selectbox("Movement Type", ["IN", "OUT"], key="chem_move_type2")

        qty = st.number_input("Quantity (kg)", min_value=0.0, step=0.1, key="chem_qty2")
        remarks = st.text_input(
            "Remarks / reference (invoice, batch, etc.)",
            key="chem_remarks2"
        )

        if st.button("💾 Save Movement", key="chem_move_save2"):
            if qty <= 0:
...
def page_cartridge():
    ...
    if after > before:
        st.warning("After-pressure cannot be higher than before. Adjust readings.")
    ...
    if dp < 1:
        ...
        st.warning("DP Warning (1–2 bar) – Monitor filter.")
    elif dp < 3:
        st.warning("DP High (2–3 bar) – Prepare to change.")
    else:
        st.error("High DP ≥ 3 bar – Replace cartridge immediately.")
